...
Neo4jCursor.prototype.forEach = function (callback, firstOnly) {
  var nodeAlias, i, rowId = -1;
  check(callback, Function);
  while (++rowId < this.cursor.length) {
    var data = {};
    if (this.cursor[rowId] && typeof this.cursor[rowId].get === 'function') {
      data = this.cursor[rowId].get();
    } else if (_.isObject(this.cursor[rowId])) {
      for (nodeAlias in this.cursor[rowId]) {
        if (_.isArray(this.cursor[rowId][nodeAlias])) {
          data[nodeAlias] = [];
          for (i = 0; i < this.cursor[rowId][nodeAlias].length; i++) {
            if (this.cursor[rowId][nodeAlias][i] && typeof this.cursor[rowId][nodeAlias][i].get === 'function') {
              data[nodeAlias].push(this.cursor[rowId][nodeAlias][i].get());
            } else {
              data[nodeAlias].push(this.cursor[rowId][nodeAlias][i]);
            }
          }
        } else if (this.cursor[rowId][nodeAlias] && typeof this.cursor[rowId][nodeAlias].get === 'function') {
          data[nodeAlias] = this.cursor[rowId][nodeAlias].get();
        } else {
          data[nodeAlias] = this.cursor[rowId][nodeAlias];
        }
      }
    }
