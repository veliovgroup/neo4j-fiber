...
(function() {
  var Fiber, Future, Match, NTRU_def, Neo4jCursor, Neo4jDB, Neo4jData, Neo4jEndpoint, Neo4jNode, Neo4jRelationship, Neo4jTransaction, URL, bound, check, events, needle, _, __error, __success, __wait, _ref, _ref1;

  _ref = require('./helpers'), events = _ref.events, _ = _ref._, URL = _ref.URL, needle = _ref.needle, __wait = _ref.__wait, bound = _ref.bound, NTRU_def = _ref.NTRU_def, __error = _ref.__error, __success = _ref.__success, Fiber = _ref.Fiber, Future = _ref.Future;
  ...
  module.exports = Neo4jDB = (function() {
    ...
    function Neo4jDB(url, opts) {
      ...
      _eb = (function(_this) {
        return function() {
          _this.emit('batch', tasks);
          return tasks = [];
        };
      })(this);
      this.on('query', (function(_this) {
        return function(task) {
          task.to = task.to.replace(_this.root, '');
          if (task.id == null) {
    ...
    Neo4jDB.prototype.__request = function(tasks) {
      return this.__call(this.__service.batch.endpoint, {
        ...
        return function(error, response) {
          if (!error) {
            return _this.__cleanUpResponse(response, function(result) {
              return _this.__proceedResult(result);
            });
          } else {
    ...
    Neo4jDB.prototype.__cleanUpResponse = function(response, cb) {
      ...
      if ((response != null ? response.data : void 0) && _.isObject(response.data)) {
        ...
        try {
          ...
          return __error("Originally received data:");
        }
      } else {
        return __error("Empty response from Neo4j, but expecting data");
      }
    };
    ...
    Neo4jDB.prototype.__batch = function(task, callback, reactive, noTransform) {
      ...
      if (!callback) {
        return __wait((function(_this) {
          return function(fut) {
            return _this.once(task.id, function(error, response) {
              return bound(function() {
                if (error) {
                  return fut["throw"](error);
                } else {
                  return fut["return"](noTransform ? response : _this.__transformData(_.clone(response), reactive));
                }
              });
            });
          };
        ...
        return this.once(task.id, (function(_this) {
          return function(error, response) {
            return bound(function() {
              return callback(error, noTransform ? response : _this.__transformData(_.clone(response), reactive));
            });
          };
        })(this));
      }
    ...
    Neo4jDB.prototype.__connect = function() {
      var endpoint, key, response, _ref2;
      response = this.__call(this.root);
              ...
              for (key in _ref2) {
                ...
                if (_.isString(endpoint)) {
                  ...
                  if (key === "neo4j_version") {
                    __success("v" + endpoint);
                  }
                }
              }
              this.emit('ready');
              return __success("Successfully connected to Neo4j on " + this.url);
            }
            break;
          default:
    ...
    Neo4jDB.prototype.__call = function(url, options, method, callback) {
      ...
      request = (function(_this) {
        return function(method, url, body, options, callback) {
          return needle.request(method, url, body, options, function(error, response) {
            var result;
      ...
      try {
        if (!callback) {
          return __wait(function(fut) {
            return request(method, url, options.data, options, function(error, response) {
              if (error) {
                return fut["throw"](error);
              } else {
        ...
        error = _error;
        return __error("Error sending request to Neo4j (GrapheneDB) server: ", error);
      }
    };
    ...
    Neo4jDB.prototype.__transformData = function(response, reactive) {
      ...
      if ((response != null ? response.data : void 0) && (response != null ? response.metadata : void 0)) {
        if ((response != null ? response.start : void 0) && (response != null ? response.end : void 0)) {
          ...
          return new Neo4Data(this.__parseNode(response), (response != null ? response.self : void 0) ? reactive : false);
        }
      }
        ...
        for (_j = 0, _len1 = response.length; _j < _len1; _j++) {
          ...
          if (_.isObject(row) && ((row != null ? row.data : void 0) || (row != null ? row.metadata : void 0))) {
            ...
            if ((row != null ? row.start : void 0) && (row != null ? row.end : void 0)) {
              ...
              result.push(new Neo4Data(this.__parseNode(row), (row != null ? row.self : void 0) ? reactive : false));
            }
          }
        }
        if (hasData) {
    ...
    Neo4jDB.prototype.__getCursor = function(task, callback, reactive) {
      if (!callback) {
        return __wait((function(_this) {
          return function(fut) {
            return _this.__batch(task, function(error, data) {
              if (error) {
                return fut["throw"](error);
              } else {
        ...
        Fiber((function(_this) {
          return function() {
            return _this.__batch(task, function(error, data) {
              return callback(error, new Neo4jCursor(data));
            }, reactive);
          };
        })(this)).run();
        return this;
    ...
    Neo4jDB.prototype.queryAsync = function(cypher, opts, callback) {
      ...
      Fiber((function(_this) {
        return function() {
          return _this.query(cypher, opts, callback);
        };
      })(this)).run();
    };
