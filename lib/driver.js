...
function Neo4jDB (url, opts) {
  ...
  if (opts.headers != null && _.isObject(opts.headers)) {
    this.defaultHeaders = _.extend(this.defaultHeaders, opts.headers);
  }

  var parsedURL = URL.parse(this.url);
  var _auth     = parsedURL.auth || opts.username + ':' + opts.password || false;

  if (_auth) {
    this.defaultHeaders.Authorization = 'Basic ' + (new Buffer(_auth).toString('base64'));
  }
...
Neo4jDB.prototype.__connect = function () {
  ...
  if (response != null && response.statusCode) {
    ...
    case 200:
      ...
      break;
    default:
      if (this.__connectAttempts < 3) {
        ++this.__connectAttempts;
    ...
    __error('Received response from ' + this.url + ': ', (response || 'Empty response'));
    ...
    if (result != null ? result.graph : void 0) {
      ...
      for (key in result.graph) {
        if (_.isArray(result.graph[key]) && result.graph[key].length > 0) {
          for (j = 0; j < result.graph[key].length; j++) {
            if (key === 'nodes') {
