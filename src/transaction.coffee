{events, _, __wait, __error} = require './helpers'
...
module.exports = class Neo4jTransaction
  ...
  constructor: (@_db, settings, opts = {}) ->
    ...
    try
      response = @_db.__call @_db.__service.transaction.endpoint, data: statements: statement.request, 'POST'
      @__proceedResults null, response, statement.reactive
      @_commitURL = response.data.commit
      @_execURL = response.data.commit.replace '/commit', ''
      @_expiresAt = response.data.transaction.expires
      @emit 'ready'
    catch error
      @__proceedResults error, null, statement.reactive
